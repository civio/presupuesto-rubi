...
def read_functional_categories(filename):
  reader = csv.reader(open(functional_categories_file, 'rb'))
  categories = {}
...
functional_categories_file = os.path.join(base_path, '..', 'data', 'ca', 'areas_funcionales.csv')
functional_categories = read_functional_categories(functional_categories_file)
...
for index, line in enumerate(reader):
  ...
  area = functional_categories[programme_id]
  economic_id = line[12]
  ...
  if line[3]!='':
    ...
    beneficiary_name = "<abbr title='Aquest concepte recull les persones físiques, la identitat " \
